...
@section('content')
    ...
    <div x-data="{
        ...
        <div class="sticky-header sticky top-0 z-20 shadow-lg p-6 rounded-xl mb-12 flex justify-between items-center">
            <h2 class="text-4xl font-bold animate-pulse">Groups List</h2>
            <div class="flex items-center space-x-6">
                ...
                <div class="relative">
                    ...
                    <div x-show="showNotifications" class="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4">
                        <p class="text-gray-700 dark:text-gray-300">New group updates available!</p>
                    </div>
                </div>
                <!-- Dark Mode Toggle -->
        ...
        <!-- Search Form -->
        <form method="GET" action="{{ route('groups.index') }}" class="mb-8 animate-fade-in-scale">
            <div class="flex max-w-lg mx-auto">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search groups..." class="w-full border border-[#D3D3D3] dark:border-gray-600 rounded-l-lg px-4 py-3 text-gray-800 dark:text-gray-200 dark:bg-gray-800 input-focus transition" />
        ...
        @if(!$groups || $groups->isEmpty())
            <p class="text-center text-gray-600 dark:text-gray-400 text-lg animate-fade-in-scale">No groups available.</p>
        @else
            <div class="cards grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8" x-ref="cardsContainer">
                @foreach ($groups as $group)
                    <div class="card shadow-lg rounded-xl p-8 border-l-4 transition-all duration-300 card-hover animate-pulse-glow animate-fade-in-scale"
                         ...
                         x-data="{ open: false }"
                         tabindex="0"
                         data-href="{{ route('groups.show', $group->id) }}"
                         @click="window.location.href='{{ route('groups.show', $group->id) }}'">
                        <div class="flex items-start space-x-6">
                            <div class="icon text-4xl text-[#28A745] dark:text-[#34D399] animate-rotate-icon">
                                <i class="fas fa-users"></i>
                            </div>
                            <div class="flex-1">
                                <h3 class="title text-xl font-semibold mb-3">{{ $group->name }}</h3>
                                <div class="footer-text text-sm text-gray-500 dark:text-gray-400 mb-2">
                                    <i class="fas fa-user-tie mr-2 text-blue-500 dark:text-blue-400"></i> Manager: {{ $group->manager->username ?? 'N/A' }}
                                </div>
                                <div class="footer-text text-sm text-gray-500 dark:text-gray-400 mb-4">
                                    <i class="fas fa-users mr-2 text-purple-500 dark:text-purple-400"></i> Employees:
                        ...
                        <div x-show="open" class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4" @click.stop>
                            <a href="{{ route('groups.edit', $group->id) }}" class="block px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded animate-fan-out" style="animation-delay: 0.1s;">Edit</a>
        ...
        <div x-data="{ open: false }" class="fixed bottom-8 right-8">
            ...
            <div x-show="open" class="absolute bottom-20 right-0 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-56">
                <a href="{{ route('groups.create') }}" class="block px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded animate-fan-out" style="animation-delay: 0.1s;">New Group</a>
            </div>
        </div>
    </div>
