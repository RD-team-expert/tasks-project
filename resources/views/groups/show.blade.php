...
@section('content')
    ...
    <div x-data="{ darkMode: $store.darkMode.isDark }" :class="{ 'dark-mode': darkMode }" class="max-w-4xl mx-auto p-6 relative">
        <div class="sticky top-0 z-20 bg-white shadow-md p-4 rounded-lg mb-8 flex justify-between items-center animate-slide-in dark:bg-gray-800">
            <h2 class="text-3xl font-bold text-gray-800 animate-pulse dark:text-gray-200">Group Details</h2>
            <div class="button-container">
                <a href="{{ route('groups.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition button-hover dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500" @mousedown="addRipple($event)">Back to Groups</a>
                <a href="{{ route('groups.edit', $group->id) }}" class="px-4 py-2 bg-[#FFB107] text-white rounded hover:bg-[#FFA000] transition button-hover dark:bg-yellow-600 dark:hover:bg-yellow-700" @mousedown="addRipple($event)">Edit</a>
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg animate-glow dark:bg-gray-700 animate-fade-in">
            <h3 class="text-xl font-semibold text-gray-800 mb-4 animate-pulse dark:text-gray-200">{{ $group->name }}</h3>
            <div class="space-y-4">
                <p class="text-gray-600 animate-slide-in dark:text-gray-300" style="animation-delay: 0.1s;">
                    <i class="fas fa-user-tie mr-2 text-blue-500 dark:text-blue-400"></i>
                ...
                <p class="text-gray-600 animate-slide-in dark:text-gray-300" style="animation-delay: 0.2s;">
                    <i class="fas fa-users mr-2 text-purple-500 dark:text-purple-400"></i>
                    <strong>Employees:</strong>
                @if($group->employees->isNotEmpty())
                    <ul class="list-disc list-inside mt-1">
                        @foreach($group->employees as $employee)
                            <li class="text-gray-600 dark:text-gray-300">{{ $employee->username }}</li>
                        @endforeach
                    </ul>
                @else
                    None
