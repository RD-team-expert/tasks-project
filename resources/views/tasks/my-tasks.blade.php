...
@section('content')
    <style>
        ...
        .no-tasks-message { background: #f3f4f6; color: #6b7280; padding: 1rem; border-radius: 0.5rem; text-align: center; }
        ...
        .dark-mode .no-tasks-message { background: #3a3a3a; color: #9ca3af; }
    </style>
    ...
    <div x-data="{ darkMode: $store.darkMode.isDark }" :class="{ 'dark-mode': darkMode }" class="max-w-3xl mx-auto p-6 relative animate-fade-in">
        <div class="sticky top-0 z-20 bg-white shadow-md p-4 rounded-lg mb-8 flex justify-between items-center animate-slide-in dark:bg-gray-800">
            <h2 class="text-3xl font-bold text-gray-800 animate-pulse dark:text-gray-200">My Tasks</h2>
        ...
        <div class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-lg shadow-lg animate-glow dark:bg-gray-700" style="animation-delay: {{ $index * 0.1 }}s;">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <p class="text-gray-700 dark:text-gray-300 animate-slide-in" style="animation-delay: 0.1s;">
                            <strong class="font-semibold text-gray-800 dark:text-gray-200">Name:</strong> {{ $task->name ?? 'Untitled Task' }}
                        </p>
                        ...
                        <p class="text-gray-700 dark:text-gray-300 animate-slide-in" style="animation-delay: 0.8s;">
                            ...
                            <span class="inline-block px-2 py-1 text-sm rounded
                                @if($task->status === 'Completed') bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-200
                                @elseif($task->status === 'In Progress') bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-200
                                @elseif($task->status === 'Not Started') bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-200
                                @endif">
                                {{ $task->status ?? 'Unknown' }}
                            </span>
                        </p>
                    </div>
                    <div class="mt-4 button-container animate-slide-in" style="animation-delay: 0.9s;">
                        <div class="relative tooltip-parent">
                            <a href="{{ $task->id ? route('tasks.editDetails', $task->id) : '#' }}"
                               ...
                               {{ !$task->id ? 'opacity-50 cursor-not-allowed' : '' }}"
                               @if($task->id) @mousedown="addRipple($event)" @endif>
                                Edit Task Details
                            </a>
            ...
            @empty
                <p class="no-tasks-message animate-fade-in dark:text-gray-400">You currently have no tasks assigned.</p>
            @endforelse
        </div>
    </div>
